var App = function () {
    ...
    var handleTables = function () {
        ...
        jQuery('#sample_4_wrapper .dataTables_length select').addClass("m-wrap xsmall"); // modify table per page dropdown
		...
		// begin: Customers tables
        $('#register_customers').dataTable({
            "sDom": "<'row-fluid'<'span6'l><'span6'f>r>t<'row-fluid'<'span6'i><'span6'p>>",
            "sPaginationType": "bootstrap",
            "oLanguage": {
                "sLengthMenu": "_MENU_ per page",
    ...
    var handleFormValidation = function () {
		...
		// Form Validation for <new_user.php> page
        var new_user = $('#new_user_form');
        var error1 = $('.alert-error', new_user);
        var success1 = $('.alert-success', new_user);
        ...
        new_user.validate({
			...
			country: "Please select country from the list",
			province: "Please select province from the list",
			state: "Please select state from the list",
			zip_code: "Please enter valid zip code",
			category: "Please select user category",
			status: "Please chose the status of the user"
			},
			
			errorPlacement: function (error, element) { // render error placement for each input type
                if (element.attr("name") == "gender") { // for uniform radio buttons, insert the after the given container
                    error.addClass("no-left-padding").insertAfter("#new_user_gender_error");
                } else if (element.attr("name") == "status") { // for uniform checkboxes, insert the after the given container
                    error.addClass("no-left-padding").insertAfter("#new_user_status_error");
                } else {
	...
	var handleFormValidation1 = function () {
		
		// Form Validation for <product_category.php> page
        var product_category = $('#product_category');
        var error1 = $('.alert-error', product_category);
        var success1 = $('.alert-success', product_category);
        ...
        product_category.validate({
			...
			messages: {
			category: "Please enter valid category name"
			},
	...
	var handleFormValidation2 = function () {
		
		// Form Validation for <product_sub_category.php> page
        var product_sub_category = $('#product_sub_category');
        var error1 = $('.alert-error', product_sub_category);
        var success1 = $('.alert-success', product_sub_category);

        product_sub_category.validate({
            errorElement: 'span', //default input error message container
            errorClass: 'help-inline', // default input error message class
            focusInvalid: false, // do not focus the last invalid input
            ignore: "",
			...
			messages: {
			category: "Please enter valid category name",
			category_name: "Please select one category from the above list"
			},
	...
	var handleFormValidation3 = function () {
		
		// Form Validation for <brand.php> page
        var brands = $('#brands');
        var error1 = $('.alert-error', brands);
        var success1 = $('.alert-success', brands);
        ...
        brands.validate({
			...
			messages: {
			brand_name: {
				required: "Please enter valid brand name",
	...
	var handleFormValidation4 = function () {
		
		// Form Validation for <product.php> page
        var product = $('#product');
        var error1 = $('.alert-error', product);
        var success1 = $('.alert-success', product);
        ...
        product.validate({
			...
			messages: {
				brand_name: {
					...
					minlenght: "Please enter at least 3 characters"
				},
				category : "Please select at least one category",
				sub_category: "Please select at lease one sub_category",
				price: "Please enter valid price for the product",
				condition: "Please select one condition from the list",
				details: {
					required: "Please write something about this product",
					minlenght: "Please enter at least 10 characters"
				},
				product_image: "Please upload at least one image",
				status: "Please select one status for the product",
				slider: "Please select status for the slider",
				stock: "This is required!",
				quantity: "Please enter quantity for this field"
			},
	...
	var handleFormValidation6 = function () {
		
		// Form Validation for <shipment.php> page
        var shipment = $('#shipment');
        var error1 = $('.alert-error', shipment);
        var success1 = $('.alert-success', shipment);
        ...
        brands.validate({
			...
			zip: {
				required: "Please type valid zip code",
				minlenght: "Zip code must grater than 4 characters"
			},
			cost: {
				required: "Please enter product shipping cost"
			},
			delivery: {
				required: "Please enter delivery time of the product"
			},
			status: {
    ...
    return {
        ...
        init: function () {
			...
			if (App.isPage("shipping")) {
                handleTables(); // handles data tables
            }
			
			if (App.isPage("user_list")) {
                handleTables(); // handles data tables
            }
			...
			if (App.isPage("customer_list")) {
                handleTables(); // handles data tables
            }
			
			if (App.isPage("product_list")) {
                handleTables(); // handles data tables
            }
            ...
            if (App.isPage("product_category")) {
                handleFormValidation1(); // handles form validation samples
				handleTables(); // handles data tables
            }
			
			if (App.isPage("product_sub_category")) {
                handleFormValidation2(); // handles form validation samples
				handleTables(); // handles data tables
            }
			
			if (App.isPage("brand")) {
                handleFormValidation3(); // handles form validation samples
				handleTables(); // handles data tables
            }
			
			if (App.isPage("product")) {
                handleFormValidation4(); // handles form validation samples
            }
			
			if (App.isPage("shipment")) {
                handleFormValidation6(); // handles form validation samples
            }
			
			if (App.isPage("new_user")) {
